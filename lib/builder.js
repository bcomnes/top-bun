...
import { SiteupAggregateError } from './helpers/siteup-aggregate-error.js'
 ...
 * @typedef {Array<EsbuildMessage | SiteupWarning>} BuildStepWarnings
 */
...
export async function builder (src, dest, opts) {
  ...
  if (siteData.errors.length > 0) {
    const pageWalkErrors = new SiteupAggregateError(siteData.errors, 'Page walk finished but there were errors.', siteData)
  ...
  if (errors.length > 0) {
    const preBuildError = new SiteupAggregateError(errors, 'Prebuild finished but there were errors.', results)
    throw preBuildError
  }
  ...
  if (errors.length > 0) {
    const buildError = new SiteupAggregateError(errors, 'Build finished but there were errors.', results)
    throw buildError
  } else {
    return results
  }
