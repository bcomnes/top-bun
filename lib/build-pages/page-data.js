...
export class PageData {
  ...
  async #renderPostVars ({ vars, styles, scripts, pages, page }) {
    ...
    const renderedPostVars = {
      ...globalVars,
      ...pageVars,
      ...(await this.postVars({ vars, styles, scripts, pages, page, workers: this.workers })),
      ...builderVars,
    }
  ...
  async renderInnerPage ({ pages }) {
    ...
    const { pageInfo, styles, scripts, vars, builderOptions } = this
    ...
    // @ts-ignore
    return await pageLayout({ vars: renderedPostVars, styles, scripts, pages, page: pageInfo, workers: this.workers })
  }
