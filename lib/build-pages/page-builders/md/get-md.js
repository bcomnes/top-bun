...
/** @type {import('../../../identify-pages.js').PageFileAsset | null} */
let markdownItSettings = null
/** @type {string | null} */
let markdownItSettingsPath = null

/**
 * @param {import('../../../identify-pages.js').PageFileAsset} settings
 * @param {string} path
 */
export function setMarkdownItSettings (settings, path) {
  markdownItSettings = settings
  markdownItSettingsPath = path
}
...
export async function getMd () {
  ...
  if (markdownItSettings && markdownItSettingsPath) {
    try {
      const settingsModule = await import(markdownItSettingsPath)
 ...
 * @param  {InstanceType<typeof markdownIt>} [md]         an instance of markdown
 * @return {Promise<string>}            Rendered markdown to html
 */
export async function renderMd (mdUnparsed, vars, md) {
  if (!md) md = await getMd()
  // @ts-ignore
