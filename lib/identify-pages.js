...
import { SiteupDuplicatePageError } from './helpers/siteup-error.js'

/**
 * @typedef {import('./builder.js').SiteupOpts} SiteupOpts
 * @typedef {import('fs').Stats} Stats
 * @typedef {import('async-folder-walker').FWStats} FWStats
 * @typedef {import('./helpers/siteup-warning.js').SiteupWarning} SiteupWarning
 */
...
export async function identifyPages (src, opts = {}) {
  ...
  for (const [dir, files] of Object.entries(dirs)) {
    ...
    if (conflict) {
      const err = new SiteupDuplicatePageError(
    ...
    for (const [fileName, fileInfo] of Object.entries(files)) {
      ...
      if (fileName.endsWith(layoutSuffix)) {
        ...
        if (layouts?.[layoutName]?.relname) {
          warnings.push(
            /** @type {SiteupWarningCode} */
            {
              code: 'SITEUP_WARNING_DUPLICATE_LAYOUT',
      ...
      if (fileName.endsWith(layoutStyleSuffix)) {
        ...
        if (layouts?.[layoutStyleName]?.layoutStyle) {
          warnings.push({
            code: 'SITEUP_WARNING_DUPLICATE_LAYOUT_STYLE',
          ...
          if (layout) {
            ...
            warnings.push({
              code: 'SITEUP_WARNING_ORPHANED_LAYOUT_STYLE',
      ...
      if (fileName.endsWith(layoutClientSuffix)) {
        ...
        if (layouts?.[layoutClientName]?.layoutClient) {
          warnings.push({
            code: 'SITEUP_WARNING_DUPLICATE_LAYOUT_CLIENT',
          ...
          if (layout) {
            ...
            warnings.push({
              code: 'SITEUP_WARNING_ORPHANED_LAYOUT_CLIENT',
  ...
  if (!layouts['root']) {
    warnings.push({
      code: 'SITEUP_WARNING_NO_ROOT_LAYOUT',
  ...
  pages = pages.filter(p => {
    if (pageBuilders[p.type]) return true
      ...
      warnings.push({
        code: 'SITEUP_WARNING_UNKNOWN_PAGE_BUILDER',
