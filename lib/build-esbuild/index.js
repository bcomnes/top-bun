...
const SITEUP_DEFAULTS_PREFIX = 'siteup-defaults'
...
export async function buildEsbuild (src, dest, siteData, _opts) {
  ...
  if (siteData.defaultLayout) {
    entryPoints.push(
      { in: join(__dirname, '../defaults/default.style.css'), out: join(SITEUP_DEFAULTS_PREFIX, 'default.style.css') },
      { in: join(__dirname, '../defaults/default.client.js'), out: join(SITEUP_DEFAULTS_PREFIX, 'default.client.js') }
    )
  }
  ...
  try {
    ...
    if (siteData.defaultLayout) {
      const defaultClient = Object.values(outputMap).find(p => /^siteup-defaults.*\.js$/.test(p))
      const defaultStyle = Object.values(outputMap).find(p => /^siteup-defaults.*\.css$/.test(p))
